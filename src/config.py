...
# Ensure all runtime storage and logging directories exist
for _p in [
    feedback_log_location.parent,
    deferred_case_location.parent,
    conversations_file_loc.parent,
    checkpoint_conn_loc.parent,
    _REQUEST_LOG_FILE.parent,
    rag_log_location.parent,
    project_root / "storage" / "mlflow",
    project_root / "storage" / "vector_databases",
]:
    _p.mkdir(parents=True, exist_ok=True)

mlflow_tracking_uri = f"sqlite:///{(project_root / 'storage' / 'mlflow' / 'traces.db').as_posix()}"