"""
FastAPI backend for the Cartflow agent.
...
app = FastAPI(title="Cartflow Agent API", version="1.0.0")
...
def _to_str_content(val: object) -> str:
    if val is None:
        return ""
    if isinstance(val, str):
        return val
    if isinstance(val, dict):
        return str(val.get("response_message") or json.dumps(val, ensure_ascii=False))
    return str(val)
...
def list_threads() -> list[ThreadSummary]:
    ...
    for tid, data in store.items():
        ...
        last_msg = None
        if messages:
            last_msg = _to_str_content(messages[-1].get("content"))
        summaries.append(
            ThreadSummary(
                thread_id=tid,
...
def get_thread_messages(thread_id: str) -> ThreadMessagesResponse:
    ...
    return ThreadMessagesResponse(
        thread_id=thread_id,
        messages=[
            Message(role=m.get("role", "user"), content=_to_str_content(m.get("content")))
            for m in history
        ],
    )
